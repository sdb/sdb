...
def quora_entry(entry, service, desc, typ, data):
  import re
  if (data['desc'].find('followed a question.') >= 0):
    return Entry(uuid=entry.link + "/follow", service=service, desc='Quora Activity', data=json.dumps(data), pub_date=datetime(*entry.updated_parsed[:6]), typ='collab')
  else:
    return None
